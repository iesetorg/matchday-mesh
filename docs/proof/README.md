...
| `matchday-mesh-invite-export-panel-2026-06-30.jpg` | P2P invite export fills the on-page textarea and summary panel. | Browser harness with a Pear-like runtime clicked `Export Invite`; DOM contained `Invite ready to share.`, invite JSON, `read-only`, and `ops 3`, with no alert dialog. |
...
| `pear-release-renderer-proof-2026-06-30.json` | Released Pear link launched and wrote renderer proof for release `1952`. | `hasPear` and `hasMatchdayAPI` were true, backend was Corestore/Hyperbee, and the invite type was `matchday-mesh-core-invite-v1`; Pear's shim warning persisted because the suggested bin directory is absent on this host. |
| `matchday-demo-flow-proof-2026-06-30.json` | Deterministic replay of the demo video flow. | Replays 8 operations, summarizes the released-link invite handoff, proves Ada checked in, and records a 5 USDt demo pool contribution. |
