...
|---|---:|---|
| Pear release must be reproducible after future edits | Medium | Release succeeded for `pear://9a5qzrbaccfqsnwmaktb6irpe1mrapq37m9uxt1wzfq3nh3d8xfy` at length `2386`. Re-run stage/release/seed after any launch-critical edit. |
...
| Live cross-device Hyperswarm pairing still needs field proof | Medium | The runtime now has Host Pairing and Join Replica controls, and tests prove a hosted topic can sync a read-only replica with live updates through an injected swarm. Capture a two-device Pear Runtime proof when another host is available. |
