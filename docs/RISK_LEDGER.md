...
| Tester handoff drifts across separate commands | Low | `npm run check:final` covers the portable final packet, while `npm run check:launch` runs the launch-workstation rehearsal with real Hyperswarm pairing and live-readiness checks when preview/catalog/seed processes are running. |
